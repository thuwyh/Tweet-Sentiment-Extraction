...
class TrainDataset(Dataset):
    ...
    def __init__(self, data, old_data, tokenizer, mode='train', smooth=False, epsilon=0.0, distill=False, offset=4):
        ...
        if 'selected_text' in self._data.columns.tolist():
            ...
            self._data['c_selected_text'] = self._data['selected_text'].apply(
                lambda x: clean(x))
            # self._data['whole_sentence'] = self._data.apply(lambda x: len(x['selected_text'])/len(x['text'])>0.95, axis=1).astype(int)
            
            # self._data['st_word_count']=self._data['c_selected_text'].apply(lambda x: len(x.split()))
            # self._data.loc[self._data['st_word_count']==1, 'whole_sentence']=2
    ...
    def prepare_word(self):
        ...
        self._data['word_invert_map'] = word_invert_map
    ...
    def __getitem__(self, idx):
        ...
        if self._distill:
            ...
            epsilon = 0.0015*len(mask)#*random.random()
