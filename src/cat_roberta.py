...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        inputs = self._tokenizer.encode_plus(
            self._sentiment[idx], self._tokens[idx], return_tensors='pt')
...
class TweetModel(nn.Module):
    ...
    def __init__(self, pretrain_path=None, dropout=0.2, config=None):
        ...
        if config is not None:
            ...
            self.bert = RobertaModel.from_pretrained(
                pretrain_path, cache_dir=None, output_hidden_states = True)
        ...
        self.ext_head = nn.Sequential(
            OrderedDict([
                ('se', nn.Linear(self.bert.config.hidden_size*2, 2))
            ])
        )
        torch.nn.init.normal_(self.ext_head.se.weight, std=0.02)
    ...
    def forward(self, inputs, masks, token_type_ids=None, input_emb=None):
        ...
        se_out = torch.cat([hidden[-1], hidden[-2]], dim=-1)
        se_out = self.ext_head(self.drop_out(se_out))
...
def main():
    ...
    if args.mode == 'train':
        ...
        config = RobertaConfig(args.bert_path+'config.json')
        ...
        for fold in range(5):
            ...
            valid_loader = DataLoader(valid_set, batch_size=args.batch_size, shuffle=False, collate_fn=partial(collate_fn, tokenizer=tokenizer),
                                      num_workers=args.workers)
            valid_pred = predict(model, valid_fold, valid_loader,
                                 args, progress=True, temperature=args.temperature)
            if args.swap:
                valid_set.swap()
                valid_pred2 = predict(
        ...
        all_senti_preds, all_start_preds, all_end_preds = predict(
            model, valid_fold, valid_loader, args)
...
def predict(model: nn.Module, valid_df, valid_loader, args, progress=False, for_ensemble=False) -> Dict[str, float]:
    ...
    with torch.no_grad():
        for inputs, types, _, _, _ in valid_loader:
            ...
            types = types.cuda()
            senti_out, start_out, end_out = model(inputs, masks, types)
            # start_out.masked_fill(~masks, -100)
            # end_out.masked_fill(~masks, -100)
            ...
            if not for_ensemble:
                ...
                for idx in range(len(senti_pred)):
                    start, end = get_best_pred(
                        start_out[idx, :], end_out[idx, :], np.sum(masks[idx, :])-2, offset=4)
