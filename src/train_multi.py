...
class MyCollator:
    ...
    def __call__(self, batch):
        ...
        inst = pad_sequence(inst, batch_first=True, padding_value=0)
        return tokens, type_ids, masks, label, start, end, inst.float(), all_sentence
...
class TweetModel(nn.Module):
    ...
    def __init__(self, pretrain_path=None, dropout=0.2, config=None):
        ...
        self.inst_head = nn.Linear(self.bert.config.hidden_size, 1)
...
def train(args, model: nn.Module, optimizer, scheduler, *,
    ...
    for epoch in range(start_epoch, start_epoch + n_epochs):
        ...
        for i, (tokens, types, masks, targets, starts, ends, inst, all_sentence) in enumerate(train_loader):
            ...
            inst_loss = bce_fn(inst_out, inst.unsqueeze(-1))
    ...
    with torch.no_grad():
        for tokens, types, masks, _, _, _, _, _ in valid_loader:
            ...
            for idx in range(len(start_out)):
                ...
                all_inst_out.append(inst_out[idx,:].cpu().flatten())
