...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        inputs = self._tokenizer.encode_plus(self._sentiment[idx], self._tokens[idx], return_tensors='pt')
...
class TweetModel(nn.Module):
    ...
    def forward(self, inputs, masks, token_type_ids=None, input_emb=None):
        seq_output, pooled_output = self.bert(
            inputs, attention_mask=masks, token_type_ids=token_type_ids, inputs_embeds=input_emb)
...
def main():
    ...
    if args.mode == 'train':
        ...
        all_start_preds, all_end_preds = predict(model, valid_fold, valid_loader, args)

        metrics = get_metrics(all_start_preds, all_end_preds, valid_fold)
...
def train(args, model: nn.Module, optimizer, scheduler, *,
    ...
    for epoch in range(start_epoch, start_epoch + n_epochs):
        ...
        losses = []
        mean_loss = 0
        for i, (inputs, types, starts, ends) in enumerate(train_loader):
            lr = get_learning_rate(optimizer)
            batch_size, length = inputs.size(0), inputs.size(1)
            masks = (inputs > 0).cuda()
            inputs, types = inputs.cuda(), types.cuda()
            ...
            loss = (start_loss+end_loss)/ args.step
...
def get_best_pred(start_pred, end_pred, text_len):
    ...
    for i in range(5):
        for j in range(5):
            if top_start[i]>2 and top_start[i]<text_len+3:  # +3 for cls token, sep and sentiment
                if top_end[j]>=top_start[i] and top_end[j]<text_len+3:
...
def predict(model: nn.Module, valid_df, valid_loader, args, progress=False) -> Dict[str, float]:
    ...
    with torch.no_grad():
        for inputs, types, _, _ in valid_loader:
            if progress:
            ...
            for idx in range(len(start_out)):
                start, end = get_best_pred(start_out[idx,:], end_out[idx,:], np.sum(masks[idx,:])-4)
                all_start_pred.append(start-3)
                all_end_pred.append(end-3)
...
def validation(model: nn.Module, valid_df, valid_loader, args, save_result=False, progress=False):
    ...
    all_start_preds, all_end_preds = predict(model, valid_df, valid_loader, args)
    ...
    metrics = get_metrics(all_start_preds, all_end_preds, valid_df)

    
    return metrics


def get_metrics(all_start_preds, all_end_preds, valid_df):
    ...
    for idx in range(len(texts)):
        ...
        if idx<5:
            print(selected_text_pred, selected_text_label)
    metrics['score'] = score/len(texts)
    print('jaccard:', metrics['score'])
    return metrics
