...
def main():
    ...
    if args.mode == 'train':
        ...
        for fold in range(5):
            ...
            word_preds, scores = get_predicts_from_token_logits(fold_whole_preds, fold_start_pred, fold_end_pred, fold_inst_preds, valid_fold, args)
...
def predict(model: nn.Module, valid_df, valid_loader, args, progress=False, softmax=False) -> Dict[str, float]:
    # run_root = Path('../experiments/' + args.run_root)
    model.eval()
